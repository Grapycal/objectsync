...
from topicsync.topic import Topic, DictTopic
...
class Server:
    def __init__(
        self,
        root_object_type: type[SObject] = SObject,
        deserialize_sort_key: Callable[[SObjectSerialized], int] = lambda x: 0,
    ) -> None:
        ...
        self._objects_topic = self.create_topic(
            "_objects", DictTopic, {root_id: "Root"}
        )
    ...
    def _destroy_object(self, id, **kwargs):
        ...
        return {
            "type": self._object_types_to_names[obj.__class__],
            "parent_id": obj.get_parent().get_id(),
    ...
    def _transition_callback(self, transition: Transition):
        ...
        for change in transition.changes:
            ...
            match split[0]:
                ...
                case "destroy_object":
                    assert isinstance(change, (EventChangeTypes.EmitChange))
                    affected_objs.append(
                        self._objects[change.forward_info["parent_id"]]
                    )
                case "a":
                ...
                case "tags":
                    affected_objs.append(
                        self._objects[change.topic_name.split("/")[1]]
                    )  # improve this
    ...
    def unregister(self, object_type: type[SObject] | str):
        ...
        for obj in self._objects.values():
            if obj.get_type_name() == object_type_name:
                raise ValueError(
                    f"Cannot unregister object type {self._object_types_to_names[object_type]} with existing object {obj.get_id()}"
                )
        del self._object_types[object_type_name]
        del self._object_types_to_names[object_type]

    def get_all_object_types(self) -> Dict[str, type[SObject]]:
    ...
    def create_object(
        ...
        **build_kwargs,
        ...
        new_object = self.create_object_s(
            self._object_types_to_names[type], parent_id, id, serialized, **build_kwargs
        )
    ...
    def create_topic(
        ...
        topic = self._topicsync.add_topic(
            topic_name,
            topic_type,
            init_value,
    ...
    def on(
        ...
        **kwargs: None,
        ...
        self._topicsync.on(
            event_name, callback, inverse_callback, is_stateful, auto=auto
        )
