...
from typing import TYPE_CHECKING, Any, Callable, Dict, List, Self, TypeVar
...
from objectsync.history import History
from objectsync.topic import (
    ObjDictTopic,
    ObjListTopic,
    ObjSetTopic,
    ObjTopic,
...
class SObjectSerialized:
    ...
    def to_dict(self):
        ...
        return {
            ...
            "children": {
                child_id: child.to_dict() for child_id, child in self.children.items()
            },
    ...
    def has_child(self, name: str) -> bool:
        return (
            name in self.user_sobject_references
            and self.user_sobject_references[name] in self.children
        )
        ...
        for attr in self.attributes:
            ...
            if name in self.wrapped_topics:
                if isinstance(value, str) and value in id_map:
                    ...
                    attr[2] = {
                        key: id_map[id] if id in id_map else id
...
class SObject:
    ...
    def __init__(self, server: Server, id: str, parent_id: str):
        self._server = server
        self._id = id
        self._parent_id = self._server.create_topic(
            f"parent_id/{id}", StringTopic, parent_id
        )
        self._tags = self._server.create_topic(
            f"tags/{id}", SetTopic, is_stateful=False
        )
    ...
    def initialize(
        self,
        serialized: SObjectSerialized | None = None,
        ...
        if serialized is None:
            ...
            for k, v in self.__dict__.items():
                if isinstance(v, Topic | WrappedTopic):
                    if v not in self._attributes.values():
                        continue
    ...
    def _deserialize(self, serialized: SObjectSerialized):
        ...
        for attr_info in serialized.attributes:
            ...
            if serialized_topic:
                ...
                self.add_attribute(
                    name, type, value, is_stateful, order_strict=order_strict
                )
        ...
        for ref_name, attr_name in serialized.user_attribute_references.items():
            if getattr(self, ref_name, None) is not None:
                continue
            assert (
                attr_name in self._attributes
            ), f"Attribute {attr_name} not found in {self.get_id()} {self.get_type_name()}"
        ...
        children = sorted(
            serialized.children.values(), key=self._server.deserialize_sort_key
        )
        for child_serialized in children:
            self._server._create_object(
                child_serialized.type, self._id, child_serialized.id, child_serialized
            )
    ...
    def restore_attribute(self, topic_name, topic_type: type[T1], serialized) -> T1:
        ...
        new_attr = self._server.restore_topic(
            f"a/{self._id}/{topic_name}", topic_type, serialized
        )
    ...
    def add_attribute(
        ...
        if origin_type == ObjTopic:
            ...
            inner = self._server.create_topic(
                f"a/{self._id}/{topic_name}",
                StringTopic,
                init_value,
                is_stateful,
            ...
            if init_value is not None:
                ...
                if len(init_value) > 0 and isinstance(
                    list(init_value.values())[0], SObject
                ):
                    init_value = {
                        key: value.get_id() for key, value in init_value.items()
                    }
            inner = self._server.create_topic(
                f"a/{self._id}/{topic_name}",
                DictTopic,
            ...
            new_attr = self._server.create_topic(
                f"a/{self._id}/{topic_name}",
                topic_type,
                init_value,
                is_stateful,
    ...
    def get_attribute(self, topic_name, topic_type: type[T5] = object) -> T5:
        if topic_name not in self._attributes:
            raise ValueError(f"Attribute '{topic_name}' does not exist")
        topic = self._attributes[topic_name]
    ...
    def emit(self, event_name, **kwargs):
        ...
        if event_name not in self._attributes:
            self._attributes[event_name] = self._server.get_topic(
                f"a/{self._id}/{event_name}"
            )
    ...
    def on(
        ...
        event_name: str,
        callback: Callable,
        inverse_callback: Callable | None = None,
    ...
    def register_service(
        self, service_name: str, callback: Callable, pass_sender: bool = False
    ):
        self._server.register_service(
            f"{self._id}/{service_name}", callback, pass_sender
        )
    ...
    def serialize(self) -> SObjectSerialized:
        ...
        children_serialized = {
            child.get_id(): child.serialize() for child in self._children
        }
